 <?php
    ...
    */
    ...
    function make_query($shelf = '4', $max = '10', $layout = '1',$startIndex = 0,$idNumber) {
                    $url = "https://www.googleapis.com/books/v1/users/".$idNumber."/bookshelves/".$shelf."/volumes?maxResults=" . "50"."&startIndex=".$startIndex;
		...
		//code by http://wordpress.org/support/profile/vadym to allow user to retrieve more than 40 books.
                    $all_books = array();
                   
                    if(file_exists($shelf) &&  (time()-filemtime($shelf)<600 )){
                      $all_books = unserialize(file_get_contents($shelf));
                    }else {        
                    $is_error=false;
                    $limit_value = 0;
                    do{
                     ...
                     $pos = strpos($books_response, $error_str);
                     if( $pos==true){
                       echo "google api error";
                       $is_error = true;
                     }else{                
                     ...
                     file_put_contents($shelf, $str_serialized);
                    }
		//end of code by Vadym
            ...
            <?php
                    ...
                    if(!empty($all_books)){
                            ...
                            foreach($results as $key => $value){
                                    ...
                                    $output .= "<a href='" . $value['infoLink'] . "' target='_blank'><img src='" . $value['imageLink'] . "'/></a>";
                                    $output .= "<p><b><a href='" . $value['infoLink'] . "' target='_blank'>" . $value['title'] . "</a></b><br>";
